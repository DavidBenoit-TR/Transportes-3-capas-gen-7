...
namespace DAL
{
    internal class metodos_Datos
    {
        ...
        {
            //varaible para controlar el resuto
            int id = 0;
            //obtenemos la cadena de conexión desde la clase configuración
            string cadenaConexion = Configuracion.CadenaConexion;
            //creamos una conexión
            // SqlConnection => Objeto de ADO
            SqlConnection con = new SqlConnection(cadenaConexion);
            try
            {
                //verificamos si la conexión está abierta
                ...
                {
                    //comando para SQL(sp, conexión)
                    //SqlCommand => Objeto de ADO
                    SqlCommand cmd = new SqlCommand(sp, con);
                    //definimos que el comando será un SP (stored Procedure)
                    cmd.CommandType = CommandType.StoredProcedure;
                    //pasamos el SP
                    cmd.CommandText = sp;
